...
namespace EduHub.Data.Entities
{
    ...
    public sealed partial class AKDDataSet : DataSetBase<AKD>
    {
        /// <summary>
        /// Data Set Name
        /// </summary>
        ...
        /// </summary>
        /// <param name="Items">Base <see cref="AKD" /> items</param>
        /// <param name="DeltaItems">Delta <see cref="AKD" /> items to added or update the base <see cref="AKD" /> items</param>
        /// <returns>A merged list of <see cref="AKD" /> items</returns>
        protected override List<AKD> ApplyDeltaItems(List<AKD> Items, List<AKD> DeltaItems)
        {
            Dictionary<string, int> Index_DEPARTMENT = Items.ToIndexDictionary(i => i.DEPARTMENT);
            HashSet<int> removeIndexes = new HashSet<int>();

            foreach (AKD deltaItem in DeltaItems)
            {
                int index;

                if (Index_DEPARTMENT.TryGetValue(deltaItem.DEPARTMENT, out index))
                {
        ...
        /// <summary>
        /// Returns SQL which checks for the existence of a AKD table, and if not found, creates the table and associated indexes.
        /// </summary>
        protected override string GetCreateTableSql()
        {
            return @"IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[AKD]') AND OBJECTPROPERTY(id, N'IsUserTable') = 1)
BEGIN
...
END";
        ...
        {
            return new AKDDataReader(Items.Value);
        }

        // Modest implementation to primarily support SqlBulkCopy
        private class AKDDataReader : IDataReader, IDataRecord
        {
            private List<AKD> Items;
            private int CurrentIndex;
            private AKD CurrentItem;

            public AKDDataReader(List<AKD> Items)
            {
                this.Items = Items;
