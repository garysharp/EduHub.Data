...
#if !EduHubScoped
        private IReadOnlyList<PE> Cache_PXKEY_PE_TAXCODE;
#endif
...
#if !EduHubScoped
        /// <summary>
        /// PE (Employees) related entities by [PX.PXKEY]-&gt;[PE.TAXCODE]
        /// Tax scale number
        /// </summary>
        public IReadOnlyList<PE> PXKEY_PE_TAXCODE
        {
            get
            {
                if (Cache_PXKEY_PE_TAXCODE == null &&
                    !Context.PE.TryFindByTAXCODE(PXKEY, out Cache_PXKEY_PE_TAXCODE))
                {
                    Cache_PXKEY_PE_TAXCODE = new List<PE>().AsReadOnly();
                }

                return Cache_PXKEY_PE_TAXCODE;
            }
        }
