            ...
            for (var i = 0; i < Headers.Count; i++) {
                switch (Headers[i]) {
                    ...
                    case "SUPER_PAID_TODATE":
                        mapper[i] = (e, v) => e.SUPER_PAID_TODATE = v == null ? (DateTime?)null : DateTime.ParseExact(v, "d/MM/yyyy h:mm:ss tt", CultureInfo.InvariantCulture);
                        break;
...
END");
        ...
        public override SqlCommand GetSqlDisableIndexesCommand(SqlConnection SqlConnection)
        {
            return new SqlCommand(
                connection: SqlConnection,
                cmdText:
@"IF EXISTS (SELECT * FROM dbo.sysindexes WHERE id = OBJECT_ID(N'[dbo].[PESH]') AND name = N'PESH_Index_TID')
    ALTER INDEX [PESH_Index_TID] ON [dbo].[PESH] DISABLE;
");
        }
        ...
        public override SqlCommand GetSqlRebuildIndexesCommand(SqlConnection SqlConnection)
        {
            return new SqlCommand(
                ...
                cmdText:
@"IF EXISTS (SELECT * FROM dbo.sysindexes WHERE id = OBJECT_ID(N'[dbo].[PESH]') AND name = N'PESH_Index_TID')
    ALTER INDEX [PESH_Index_TID] ON [dbo].[PESH] REBUILD PARTITION = ALL;
");
        }
